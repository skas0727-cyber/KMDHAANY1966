...
// [name, description, detail link ("" = price given after consultation)]
const BOOSTERS = [
  ["연어주사", "연어에서 얻은 DNA 성분인 PDRN에 성장인자(MGF)를 더해 진피층에 전달하는 시술입니다. 섬유아세포의 활동과 콜라겐 생성을 도와, 모공, 잔주름, 얼굴의 붉은 기, 유수분 균형 관리에 쓰입니다.", ""],
  ["PN주사", "연어 DNA에서 추출한 성분(PN)으로 만든 재생 촉진제를 진피층에 주입하는 시술입니다. 자극으로 얇아진 진피를 도톰하게 채워 피부 장벽과 유수분 균형을 돕습니다. 모공과 안색이 정돈되는 효과도 기대할\u00A0수\u00A0있습니다.", "/program/pn-booster"],
];
      ...
      <dl className="sub-price">
        {p.options.map((o) => (
          <div key={o.name}>
            ...
            <dd>{o.orig && <del>{o.orig}</del>}{o.price}</dd>
          </div>
        ))}
      </dl>
    </div>
...
export default function Skin() {
  return (
    <main className="skin-clinic">
      <SubHero img="/img/skin/hero.jpg" wide="/img/skin/hero-wide.jpg" pos="50% 20%" tabs={TABS}>
        <h1><span className="sub-label">논산 피부 한의원</span> 건강한 피부,<br /> <b>무리하지 않고 관리합니다</b></h1>
      ...
      <Notice title="비용 및 시술 안내" items={[
        "모든 시술 비용은 VAT 별도입니다.",
        "첫 방문 체험가는 첫 방문 시 1회에 한해 적용됩니다.",
        "점과 잡티 제거는 같은 병변이 남아 있으면 12주 이내 리터치가 포함됩니다.",
        "시술 결과에는 개인차가 있으며, 붉어짐, 붓기, 딱지 등 일시적인 반응이 생길\u00A0수\u00A0있습니다.",
        `가격은 병원 사정에 따라 바뀔\u00A0수\u00A0있으니 ${TEL}로 확인해 주세요.`,
      ]} />
