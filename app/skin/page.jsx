...
export const metadata = pageMeta(
  ...
  "논산 강경 피부 한의원 광명당한의원. 기미·검버섯·비립종·편평사마귀·오타모반·한관종 같은 색소·잡티 고민을 야누스프로 피부 분석 후 관리합니다. 슈링크 리프팅, 연어주사(PDRN)·PN주사 스킨부스터, 토닝·점 제거 가격과 첫 방문 체험가(슈링크 100샷 15,000원)를 안내합니다."
);
...
const CONDITIONS = [
  ...
  ["biripjong", "비립종", "빠져나가지 못한 각질이 피부 얕은 곳에 갇혀 하얗고 작은 알갱이처럼 보이는 주머니입니다. 눈가에 잘 생기고, 피부 턴오버가 원활하지 않으면 다른 자리에 다시 올라오기도 합니다."],
  ...
  ["hangwanjong", "한관종", "땀샘을 이루는 조직이 지나치게 자라 생기는 양성 종양으로, 눈 밑에 살색의 작은 알갱이들이 모여 올라옵니다. 재발이 잦은 편이라 주변 조직의 자극을 줄이며 신중하게 접근하는 것이 좋습니다."],
];
const POINTS = [
  ["POINT 01", "532nm · 1064nm 두 파장", "얕은 색소와 깊은 색소에 맞춰 파장을 달리하며, 멜라닌 색소를 선택적으로 겨냥합니다."],
  ["POINT 02", "수분에 반응하는 미세 박피", "피부 속 수분에 잘 흡수되는 파장으로 표면을 얇고 정교하게 다듬어, 주변 조직에 전해지는 열을 줄입니다."],
  ["POINT 03", "기화 방식의 병변 제거", "병변 조직의 수분을 순간적으로 증발시켜 제거하는 방식으로, 필요한 깊이까지 다가가면서 지혈을 돕습니다."],
];
...
const BOOSTERS = [
  ["#PDRN", "연어주사", "연어에서 얻은 DNA 성분인 PDRN에 성장인자(MGF)를 더해 진피층에 전달하는 시술입니다. 섬유아세포의 활동과 콜라겐 생성을 도와 모공·잔주름, 얼굴의 붉은 기, 유수분 균형 관리에 도움이 될 수 있습니다."],
  ["#PN", "PN주사", "연어 DNA에서 추출한 성분(PN)으로 만든 재생 촉진제를 진피층에 채워 넣는 시술입니다. 자극으로 얇아진 진피를 도톰하게 가꾸어 피부 장벽과 유수분 균형을 돕고, 모공과 안색이 정돈되는 효과를 기대할 수 있습니다.", "/program/pn-booster"],
];
  ...
  return (
    <main>
      ...
      <ul className="sub-points">
        {VALUES.map(([k, v]) => <li key={v}><span>{k}</span><b className="gm">{v}</b></li>)}
      </ul>

      <nav className="sub-tabs">
        <ul>
          {TABS.map(([id, t]) => <li key={id}><a href={"#" + id}>{t}</a></li>)}
        </ul>
      </nav>
      ...
      <section id="diagnosis" className="sub-cat">
        <Head en="SKIN ANALYSIS" title="피부 분석으로 시작하는 색소 치료" lead="눈으로 보이는 것만으로 판단하지 않습니다. 피부 상태를 숫자로 확인한 뒤 치료 계획을 세웁니다." />
        <div className="sc-intro" data-aos>
          <div>
            <p className="en gm">JANUS PRO</p>
            <h3>야누스프로 피부 분석</h3>
          </div>
      ...
      <section id="lifting" className="sub-cat">
        ...
        <div className="sc-intro" data-aos>
          <div>
            <p className="en gm">SHURINK</p>
            <h3>슈링크 리프팅</h3>
          </div>
          <div>
            <p className="desc">초음파 에너지를 한 지점에 모아 피부 아래 근막층(SMAS)까지 전달하는 리프팅 시술입니다. 근막층을 탄탄하게 자극해 처진 라인을 정돈하고, 탄력과 잔주름 관리에 도움이 될 수 있습니다.</p>
            <a className="sc-more" href="/program/shurink">슈링크 가격·자세히 보기 →</a>
          </div>
        </div>
      </section>
        ...
        <ul className="sub-cards sc-boost" style={{ "--cols": 2 }} data-aos>
          {BOOSTERS.map(([tag, name, desc, href]) => (
            <li key={tag}>
              ...
              <h3>{name}</h3>
              <p>{desc}</p>
              {href ? <a className="sc-more" href={href}>PN 스킨부스터 가격·자세히 보기 →</a> : <p className="sc-note">비용은 진료 후 안내해 드립니다.</p>}
            </li>
          ))}
        </ul>
      </section>
      ...
      <section id="price" className="sub-cat">
        <Head en="PRICE" title="시술 가격표" lead="모든 가격은 VAT 별도이며, 시술별 자세한 내용은 각 메뉴에서 볼 수 있습니다." />
      ...
      <section className="sub-notice">
        ...
        <ul>
          ...
          <li>점·잡티 제거는 같은 병변이 남아 있으면 12주 이내 리터치가 포함됩니다.</li>
      ...
      <section className="sub-cta">
        ...
        <p>첫 방문 피부진단 후 피부 상태에 맞춰 안내해 드립니다.</p>
